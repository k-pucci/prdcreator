export default async function handler(req, res) {
  ...
  if (!process.env.CLAUDE_API_KEY) {
    ...
    return generateTemplatePRD(formData, res);
  }

  // Try Claude API first
  try {
    const claudeResponse = await generateClaudePRD(formData);
...
// Claude API function
async function generateClaudePRD(formData) {
  ...
  **User Testing:** Feedback collection with target users
  ...
  IMPORTANT: Replace ALL bracketed placeholders with specific, detailed content based on the 8 inputs provided. Be concrete and actionable rather than generic. Extract specific details from each input and organize them appropriately within the template structure.`;
...
function generateTemplatePRD(formData, res) {
  const projectName = formData.question1.split(" ").slice(0, 3).join(" ");

  const templatePRD = `# ${projectName} - Summary, Scope, and Key Decisions
  ...
  **Participants:** Product Team, Engineering Team, Design Team
  ...
  ### Product Strategy
  **Target Audience:** ${
    formData.question2.split(" who ")[0] || formData.question2
  }
  **Core Problem:** ${
    formData.question2.includes(" who ")
      ? formData.question2.split(" who ")[1]
      : "Improving efficiency and user experience"
  }
  **Value Proposition:** ${formData.question1} that delivers ${
    formData.question3.split("Success")[0]
  }
  **Success Definition:** ${
    formData.question3.includes("Success")
      ? formData.question3.split("Success")[1].replace("measured by", "").trim()
  ...
  #### User Journey (Multi-Step Process)
  **Discovery & Onboarding**
  - Users discover the solution through ${
    formData.question2.includes("managers")
      ? "management channels"
      : "organic adoption"
  }
  ...
  **Core Usage & Engagement**
  - Primary workflow involves ${
    formData.question3.split(",")[0] || "core feature utilization"
  }
  - Users engage with ${
    formData.question3.split(",")[1] || "key functionality"
  } regularly
  ...
  ### Core Features
  **${
    formData.question3.split(",")[0] || "Core Feature 1"
  }:** Primary functionality that addresses user needs
  **${
    formData.question3.split(",")[1] || "Feature 2"
  }:** Secondary feature that enhances user experience
  **${
    formData.question3.split(",")[2] || "Integration Feature"
  }:** Connects with existing workflows
  **${
    formData.question3.split(",")[3] || "Analytics Feature"
  ...
  **Key Integrations:** ${
    formData.question4.includes("integrates")
      ? formData.question4.split("integrates")[1]
  ...
  **Development Approach:** ${
    formData.question6.includes("Phase")
  ...
  **Integration Strategy:** ${
    formData.question4.includes("backend")
      ? "Backend-first integration"
      : "Frontend-driven implementation"
  }
  ...
  **Total:** ${
    formData.question6.includes("weeks")
      ? formData.question6
          .match(/\d+\s*weeks?/g)
          ?.reduce((acc, curr) => acc + parseInt(curr), 0) + " weeks"
      : "12-16 weeks"
  }
  ...
  **Technology Stack:** ${formData.question4}
  **Integration Points:** ${
    formData.question4.includes("API")
  ...
  **Performance Requirements:** Sub-second response times and 99.9% uptime
  ...
  ### Data & Storage
  **Data Models:** User profiles, activity logs, ${
    formData.question3.includes("tracking")
      ? "tracking data"
      : "application data"
  }
  **Storage Requirements:** ${
    formData.question4.includes("database")
      ? formData.question4.split("database")[0] + "database"
      : "Scalable cloud database solution"
  }
  **Security Considerations:** ${
    formData.question8.includes("compliance")
      ? "Industry compliance requirements"
      : "Standard security protocols and data encryption"
  }
  ...
  **User Interface:** ${
    formData.question4.includes("React")
  ...
  ### Out of Scope / Not Supported
  **${formData.question5.split(",")[0] || "Advanced Features"}:** ${
    formData.question5.split(",")[0] || "Complex functionality"
  } excluded for initial release
  - Focus on core functionality first
  - Advanced features planned for future phases
  
  **${formData.question5.split(",")[1] || "Platform Extensions"}:** ${
    formData.question5.split(",")[1] || "Additional platforms"
  } not included
  - Web-first approach for initial launch
  - Platform expansion in subsequent releases
  
  **${formData.question5.split(",")[2] || "Enterprise Features"}:** ${
    formData.question5.split(",")[2] || "Complex enterprise functionality"
  } deferred
  ...
  **Description:** Nice-to-have features for future consideration
  - ${
    formData.question5.includes("mobile")
      ? "Mobile applications"
      : "Platform extensions"
  }
  ...
  **Operational Efficiency:** ${
    formData.question8.includes("efficiency")
      ? "Improve operational efficiency"
      : "Streamline business processes"
  }
  **User Experience:** ${
    formData.question8.includes("user")
  ...
  **Technology Modernization:** ${
    formData.question4.includes("modern")
      ? "Leverage modern technology stack"
      : "Update technology infrastructure"
  }
  **Process Optimization:** ${
    formData.question2.includes("struggle")
      ? "Address current process inefficiencies"
      : "Optimize existing workflows"
  }
  **Competitive Advantage:** ${
    formData.question3.includes("measured")
      ? "Measurable competitive improvements"
  ...
  **Technical Value:** ${
  ...
  - ${
    formData.question6.includes("weeks")
      ? formData.question6
          .match(/\d+\s*weeks?/g)
          ?.reduce((acc, curr) => acc + parseInt(curr), 0) + " week"
      : "12-16 week"
  } delivery window with agile methodology
  ...
  **User Testing:** Regular feedback sessions with target users
  ...
  - **Technical Architecture:** ${
    formData.question7.includes("API")
      ? "API integration approach"
      : "Final technical implementation details"
  }
  - **Integration Scope:** ${
    formData.question7.includes("integration")
      ? "Integration complexity and timeline"
      : "Third-party service connections"
  }
  - **Resource Allocation:** ${
    formData.question7.includes("adoption")
  ...
  ### Risk Mitigation
  - **Technical Risks:** ${
    formData.question7.split(",")[0] || "Implementation complexity"
  ...
  Strong consensus on the phased development approach, which balances early value delivery with manageable complexity and allows for iterative feedback incorporation while maintaining focus on ${
    formData.question8
  }.`;
